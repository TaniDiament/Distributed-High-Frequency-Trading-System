 ...
 */
public class ReferenceData implements Runnable{
     ...
     */
    public ReferenceData(InetSocketAddress address, URI bloombergURI, InetSocketAddress LamportAddress, List<InetSocketAddress> peers) {
        this.address = address;
        this.bloombergURI = bloombergURI;
        this.lamportAddress = LamportAddress;
        this.log = new File("ReferenceData.log"); // log file for market data
    ...
    private void lead(){
        //while (true) {
        // get reference data from bloomberg via bloombergURI TCP messages
     ...
     * @param ReferenceData the market data to be added.
     */
    public void addReferenceData(String ReferenceData) {
        //Write the reference data to the log file
        // Event event = new Event(REFERENCE_DATA, ReferenceData);
        // event.setTimestamp(System.currentTimeMillis());
        // send the market data to the lamport service via sendReferenceData(event)
    }

    /**
     * Send market data to the lamport service.
     * This will be called to send the latest market data.
     * @param ReferenceData the market data event to be sent.
     */
    public void sendReferenceData(Event ReferenceData) {
        // byte[] data = ReferenceData.toBinary();
        // send the data to the lamport service via TCP
    }
